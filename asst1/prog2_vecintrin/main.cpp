...
void clampedExpVector(float* values, int* exponents, float* output, int N) {
  ...
  __cs149_mask mask_is_zero, mask_is_not_zero;
  __cs149_mask mask_greater_than_max;
  int i;
  int r = N - VECTOR_WIDTH;

  for (i = 0; i <= r; i += VECTOR_WIDTH) {
    x = _cs149_vset_float(0);
    y = _cs149_vset_int(1);
    result = _cs149_vset_float(0);

    _cs149_vload_float(x, values + i, mask_all);
    _cs149_vload_int(y, exponents + i, mask_all);

    _cs149_veq_int(mask_is_zero, y, zero, mask_all);
    _cs149_vset_float(result, 1.f, mask_is_zero);

    mask_is_not_zero = _cs149_mask_not(mask_is_zero);
    _cs149_vsub_int(y, y, one, mask_is_not_zero);
    _cs149_vmove_float(result, x, mask_is_not_zero);
    _cs149_vgt_int(mask_is_not_zero, y, zero, mask_is_not_zero);
    while (_cs149_cntbits(mask_is_not_zero)) {
      _cs149_vmult_float(result, result, x, mask_is_not_zero);
      _cs149_vsub_int(y, y, one, mask_is_not_zero);
      _cs149_vgt_int(mask_is_not_zero, y, zero, mask_is_not_zero);
    }

    _cs149_vgt_float(mask_greater_than_max, result, max_value, mask_all);
    _cs149_vmove_float(result, max_value, mask_greater_than_max);
...
float arraySumVector(float* values, int N) {
  ...
  __cs149_mask mask_all = _cs149_init_ones();
  __cs149_mask mask_first = _cs149_init_ones(1);

  for (int i = 0; i < N; i += VECTOR_WIDTH) {
    _cs149_vload_float(tmp, values + i, mask_all);
    _cs149_vadd_float(sum, sum, tmp, mask_all);   
  }
  ...
  float result;
  _cs149_vstore_float(&result, sum, mask_first);
